...
def write_to_file(dc, ac, blocks_count, tables):
    f = ''

    for table_name in ['dc_y', 'ac_y', 'dc_c', 'ac_c']:

        # 16 bits for 'table_size'
        f += uint_to_binstr(len(tables[table_name]), 16)
        ...
        for key, value in tables[table_name].items():
            if table_name in {'dc_y', 'dc_c'}:
                ...
                f += uint_to_binstr(key, 4)
                f += uint_to_binstr(len(value), 4)
                f += value
            else:
                ...
                # 'code_length' bits for 'huffman_code'
                f += uint_to_binstr(key[0], 4)
                f += uint_to_binstr(key[1], 4)
                f += uint_to_binstr(len(value), 8)
                f += value

    # 32 bits for 'blocks_count'
    f += uint_to_binstr(blocks_count, 32)
    ...
    for b in range(blocks_count):
        for c in range(3):
            ...
            f += dc_table[category]
            f += int_to_binstr(dc[b, c])

            for i in range(len(symbols)):
                f += ac_table[tuple(symbols[i])]
                f += values[i]
...
def main():
    ...
    input_file = "img/img.jpg"
    output_file = "img/result.jpeg"

    image = Image.open(input_file)
    ycbcr = image.convert('YCbCr')

    npmat = np.array(ycbcr, dtype=np.uint8)
    npmat = npmat[:(npmat.shape[0] // 8) * 8, :(npmat.shape[1] // 8) * 8, :]
    ...
    binstr = write_to_file(dc, ac, blocks_count, tables)
    print(binstr)

    binary_file = open(output_file, 'wb')
    b = BitArray(bin=binstr)
